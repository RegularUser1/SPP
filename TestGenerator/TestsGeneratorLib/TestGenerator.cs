        ...
        )
        {
            ...
            foreach (var methodDeclaration in methodDeclarations)
            {
                ...
                if (!IsVoid(methodReturnType))
                {
                    ...
                    var assertEqalStatement = SyntaxFactory.ParseStatement("Assert.Equal(expected, actual);");
                    if (assertEqalStatement is ExpressionStatementSyntax asseerEqalExpressionSyntax)
                    {
                        methodAssignments.Add(asseerEqalExpressionSyntax);
                    }
                }
